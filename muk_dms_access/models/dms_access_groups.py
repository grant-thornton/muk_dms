...
class DocumentGroups(dms_base.DMSModel):
    ...
    default_group_for_users = fields.Boolean(
        string="Default group for users")
    ...
    def trigger_computation(self, fields, refresh=True, operation=None):
        super(DocumentGroups, self).trigger_computation(
            fields, refresh, operation)
        values = {}
        if "users" in fields:
            values.update(self.with_context(
                operation=operation)._compute_users(write=False))
        if values:
            self.write(values)
            if "users" in fields:
