...
class File(dms_base.DMSModel):
    ...
    abbreviation_code = fields.Char(
        ...
        search='_search_abbreviation_code')
    ...
    def trigger_computation(self, fields, refresh=True, operation=None):
        ...
        if "settings" in fields:
            values.update(self.with_context(
                operation=operation)._compute_settings(write=False))
        if "path" in fields:
            values.update(self.with_context(
                operation=operation)._compute_path(write=False))
            values.update(self.with_context(
                operation=operation)._compute_relational_path(write=False))
        if "extension" in fields:
            values.update(self.with_context(
                operation=operation)._compute_extension(write=False))
        if "mimetype" in fields:
            values.update(self.with_context(
                operation=operation)._compute_mimetype(write=False))
        if "index_content" in fields:
            values.update(self.with_context(
                operation=operation)._compute_index(write=False))
        if values:
            self.write(values)
            if "settings" in fields:
    ...
    def _compute_index(self, write=True):
        def get_index(record):
            type = record.mimetype.split(
                '/')[0] if record.mimetype else record._compute_mimetype(write=False)['mimetype']
            ...
            if type and type.split('/')[0] == 'text' and record.content and index_files:
                words = re.findall(
                    "[^\x00-\x1F\x7F-\xFF]{4,}", base64.b64decode(record.content))
                return ustr("\n".join(words))
            else:
                return None
        if write:
    ...
    @api.multi
    @api.depends('directory.abbreviation', 'directory')
    def _compute_abbreviation_code(self):
        for file in self:
            code = file.code or ''
            if file.directory.abbreviation:
                file.abbreviation_code = \
                    file.directory.abbreviation + '/' + code
            else:
                file.abbreviation_code = 'DOC/' + code
    ...
    def _check_name(self):
        ...
        duplicates = [rec for rec in childs if rec[1]
                      == self.name and rec[0] != self.id]
        if duplicates:
    ...
    def copy(self, default=None):
        ...
        default.update({'name': self.unique_name(
            self.name, names, self.extension)})
    ...
    def _update_reference_type(self):
        ...
        if self.reference and self.settings.save_type != self.reference.type():
            reference = self._create_reference(
                self.settings, self.directory.path, self.name, self.content)
            self._unlink_reference()
