...
class Directory(dms_base.DMSModel):
    ...
    def trigger_computation_down(self, fields, operation=None):
        ...
        for child in self.child_directories:
            child.with_context(is_subnode=True).trigger_computation(
    ...
    def trigger_computation(self, fields, refresh=True, operation=None):
        ...
        if "size" in fields:
            values.update(self.with_context(
                operation=operation)._compute_size(write=False))
        if values:
            self.write(values)
    ...
    def _check_name(self):
        ...
        if self.is_root_directory:
            childs = self.settings.root_directories.mapped(
                lambda rec: [rec.id, rec.name])
        else:
            childs = self.parent_directory.child_directories.mapped(
                lambda rec: [rec.id, rec.name])
    ...
    def copy(self, default=None):
        ...
        if self.is_root_directory:
            ...
            parent_directory = self.env['muk_dms.directory'].browse(
