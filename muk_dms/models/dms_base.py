...
class DMSBaseModel(models.BaseModel):
    ...
    def refresh(self):
        self.env['bus.bus'].sendone("%s_refresh" %
                                    self.env.cr.dbname, self._name)
...
#        return result
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        args, offset, limit, order, count = self._before_search(
            args, offset, limit, order, count)
        result = super(DMSBaseModel, self).search(
            args, offset, limit, order, count)
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        name, args, operator, limit = self._before_name_search(
            name, args, operator, limit)
        result = super(DMSBaseModel, self).name_search(
            name, args, operator, limit)
    ...
    def lock(self, user=None, refresh=False, operation=None):
        ...
        for record in self:
            ...
            if lock and lock.operation and lock.operation == operation:
                ...
                raise AccessError(
                    _("The record is locked, so it can't be locked again."))
            else:
    ...
    def _checking_lock_user(self):
        for record in self:
            ...
            if lock and lock.locked_by_ref and not lock.locked_by_ref != self.env.user:
                raise AccessError(
                    _("The record is locked by a user, so it can't be changes or deleted."))
    ...
    def user_lock(self):
        ...
        if lock:
            if lock.locked_by_ref:
                raise AccessError(
                    _("The record is already locked by another user. (%s)") % lock.locked_by_ref.name)
            else:
